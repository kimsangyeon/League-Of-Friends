...
const Detail: React.FC<Detail> = ({match}) => {
  const {kills, deaths, assists, win, championName, champLevel, gameCreation} =
    match;
  const items = [
    match.item0,
    match.item1,
    ...
    <div className={styles.detail}>
      <div className={win ? styles.win : styles.lose}>
        ...
        <div className={styles.detailMatchInfoWrap}>
          <span className={win ? styles.winText : styles.loseText}>
            {win ? '승리' : '패배'}
          </span>
          {match && (
            <div className={styles.daysAgo}>
              {getDaysAgo(new Date(gameCreation as number))}
            </div>
          )}
        </div>
        <div className={styles.killDeathsAssists}>
          <span>{kills}</span> / <span className={styles.deaths}>{deaths}</span>{' '}
          / <span>{assists}</span>
        </div>
