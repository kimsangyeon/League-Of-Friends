...
const useMatchScoreInfoList = (matchList: MatchInfo[][] = [], summonerList: SummonerInfo[] = []) => {
  return useMemo(() => matchList?.map(matchs => {
    const summoner = summonerList?.find(summoner => matchs.every(({participants}) => participants.some(({puuid}) => summoner.puuid === puuid)));
    if (!summoner) return {name: '', kills: 0, deaths: 0, assists: 0, win: 0, lose: 0};

    const {puuid, name} = summoner;

    return matchs.reduce((info, match) => {
